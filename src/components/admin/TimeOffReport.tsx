...
import { ArrowLeft, Filter, User, Download } from "lucide-react";
...
import {
  TimeOffRequestWithProfile,
  useAllTimeOffRequests,
} from "@/hooks/useAllTimeOffRequests";
import { format } from "date-fns";

interface TimeOffReportProps {
  onBack: () => void;
}

export const TimeOffReport = ({ onBack }: TimeOffReportProps) => {
  const { requests, loading } = useAllTimeOffRequests();
  ...
  const getEmployeeName = (request: TimeOffRequestWithProfile) => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-lg">Loading time off report...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Time Off Requests ({filteredRequests.length})</CardTitle>
          </CardHeader>
          <CardContent>
