drop function if exists "public"."get_pending_shifts"(target_user_id uuid);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.get_weekly_hours(target_user_id uuid)
 RETURNS TABLE(week_start_date date, monday_hours numeric, tuesday_hours numeric, wednesday_hours numeric, thursday_hours numeric, friday_hours numeric, total_week_hours numeric, all_verified boolean)
 LANGUAGE sql
AS $function$
WITH base_data AS (
	SELECT
