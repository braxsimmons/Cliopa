...
alter table "public"."time_entries" add column "verified" timestamp with time zone;

set check_function_bodies = off;


CREATE OR REPLACE FUNCTION public.get_recent_shifts(target_user_id uuid, num_shifts numeric)
 RETURNS TABLE(id uuid, start_time timestamp with time zone, end_time timestamp with time zone, team text, status text, total_hours numeric, verified timestamp with time zone)
 LANGUAGE sql
AS $function$
WITH base_data AS (
	SELECT
